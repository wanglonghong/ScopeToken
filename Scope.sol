...
contract ScopeToken is Context, IERC20, Ownable {
    ...
    string private _name = "NoScope";
    string private _symbol = "SCOPE";
    ...
    constructor (address treasuryAddress) public {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x3F29C1B9c6B58e764cad6Bbc489727B20a03b5B5);
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        uint256 tBurn = tOtherFee.mul(_burnFee).div(otherFee);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tTreasury) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tBurn, tTreasury, _getRate());

        uint256 tOtherFee = tLiquidity.add(tBurn).add(tTreasury);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        tTransferAmount = tTransferAmount.sub(tBurn).sub(tTreasury);
        return (tTransferAmount, tFee, tLiquidity, tBurn, tTreasury);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tTreasury, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function _getOtherFee() private view returns (uint256) {
        uint256  otherFee = _previousLiquidityFee.add(_previousBurnFee).add(_previousTreasuryFee);
        return otherFee;
    }
}
